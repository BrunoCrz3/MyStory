...
| 16 | **Una regeneración detenida deja la novela `Detenida`, que es terminal**, con los capítulos afectados de la versión publicada marcados `Obsoleto` y restos en la versión objetivo; y la API **acepta** una solicitud de cambio sobre esa novela, cuya confirmación apuntaría otra vez a la misma versión objetivo | Visto en el ensayo (RI-033). Decidir qué pasa tras una regeneración fallida —volver a `Publicada` y deshacer los `Obsoleto`, o una versión objetivo nueva— toca la ontología (`domain-knowledge.md` § Estados) | Pendiente de decisión del desarrollador |
...
| 18 | **Comprobar los scores por validador en la interfaz de Langfuse**: la API de scores devuelve 410 en esta organización (v1 y v2) | Las observaciones v2 sí confirman sesión, entorno, spans por rol y `hash_prompt` | — |
| 19 | El resto de la lista post-demo de la spec: servidor MCP propio, agente de seguridad, SSE, PO-11 y PO-12 | — | `specs/spec1.md` § 7 |
