...
| Nacimiento previo | formal-Lean | gate de publicación | `lean_nacimiento` | O-66 |
...
**Dirección de cada score** (TO-058). No todos se leen igual en Langfuse ni en la tabla
brief × validador:

| Score | Qué vale | Bueno |
| --- | --- | --- |
| `invencion_destinatario`, `temas_excluidos` | **Recuento** de problemas: afirmaciones inventadas, temas excluidos que aparecen | **0**. Cualquier valor mayor suspende: su umbral es cero y cierran el paso siempre |
...
`parcial`. La última columna cuenta los `NADIE`, no los puntos ciegos: **hay dieciséis** en
...
| 16 | **Una promesa pagada puede no tener apertura en la versión que se lee.** En una regeneración dirigida, si el capítulo reescrito no reabre una promesa que paga uno no afectado, la regla 3 de TO-047 la deja viva y pagada; su apertura es la de la fila vieja, que la versión nueva ya no lee | O-41, O-65, O-62 | El lector ve pagar algo que la versión nueva nunca le planteó. `cierre_arco` no protesta porque la promesa está pagada; lo único que lo previene es la regla 1 —el redactor recibe la promesa como `[abrir]`— | El validador de O-41, **post-demo** (decisión del desarrollador, 2026-09-25): toda promesa pagada tiene su apertura en una fila vigente anterior al pago. Con él, el caso volvería al redactor como intento fallido |
...
| **O-02** · El nombre del destinatario y los de los personajes se escriben exactamente como en la story bible | `alcance` §5a; `definitions.md` Capa 4 | Pruebas basadas en propiedades | T | sí | hook de capítulo | `nombres_exactos` | obligatorio | bajo | Compara contra la lista de nombres; un diminutivo legítimo que el brief no declaró se marca como error, un nombre ausente no se detecta y un nombre escrito solo con otras mayúsculas no se marca, porque los nombres suelen ser palabras comunes («Boya», «Varadero»; A-60). Un marcador en mayúsculas entre corchetes (`[NOMBRE_ANONIMIZADO]`) en lugar de un nombre también falla (TO-056) | O-27 | `quality/`, `tests/quality/` |
...
| **O-34** · Ninguna `Promesa narrativa` queda `Pendiente` al terminar la novela | `alcance` § contexto del cliente, «finales abruptos»; `config/thresholds.yaml` § continuidad = 0; pregunta 18 | Pruebas basadas en propiedades | T | sí | aceptación del último capítulo, antes de consolidar (TO-056), y gate de publicación | `cierre_arco` | obligatorio | bajo | Cuenta las promesas registradas: una expectativa que el texto abre y el extractor no registró como promesa no cuenta, y es justo la que deja el final abrupto | § Puntos ciegos #1, O-35 | `canon/`, `quality/` |
...
| **O-36** · Cada capítulo tiene gancho de cierre, salvo el último | `definitions.md` Capa 1B, atributo `gancho de cierre` | Pruebas basadas en propiedades + verificación multiagente | T, D | parcial | rol editor (criterio `arco` del judge; TO-060) | `cierre_arco` | recomendado | bajo | Comprueba que el atributo está relleno y que el último párrafo lo realiza; un gancho registrado y flojo cumple | O-35 | `quality/`, `novel/` |
...
| **O-41** · Toda `Promesa narrativa` pagada tiene su apertura en una **fila vigente de la versión** anterior al pago (el antiguo P-45) | `definitions.md` Capa 2; `domain-knowledge.md` § Modelo de canon; TO-047 | Pruebas basadas en propiedades | T | sí, **post-demo** | gate de publicación | `cierre_arco` (mitad programática, post-demo) | recomendado | bajo | **Hoy no corre** (punto ciego #16): el gate cuenta pendientes y no mira el orden. Cuando corra, comprobará el orden entre filas de la versión; un pago que el lector no reconoce como tal lo cumplirá igual | O-35, O-65 | `canon/` |
...
| **O-45** · La cadena causal avanza: cada capítulo mueve al menos un `Hilo de trama` o paga una promesa | `alcance` § contexto del cliente; § Soluciones pícaras #6 | Pruebas basadas en propiedades | T | sí | rol editor (criterio del judge; TO-060) | `ritmo` | recomendado | medio | Es el proxy estructural de la causalidad: lee si el capítulo movió algo, no si lo movió *por tanto* en vez de *y entonces*. El residuo está en § Lo que no se puede verificar | O-40, § Lo que no se puede verificar | `canon/`, `quality/` |
...
`docs/red-team.md` recibe un caso por cada intento adversarial, venga de B1, del
...
Los dieciséis de § Puntos ciegos sin cubrir. Cinco tienen validador que los cubre a medias y
...
**`formal/` no existe.** `docs/red-team.md` existe desde el ensayo de la demo (RT-001, RT-002), el
browser MCP del agente está en `.mcp.json` en lugar de `.claude/mcp.json` (TO-051), y `docs/browser-mcp.md` y
